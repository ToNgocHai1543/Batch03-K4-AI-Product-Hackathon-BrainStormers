...
import { sampleBridgeBundle, sampleQuizSet, BANK_STATS } from '../data/bridgeSampler.js';
...
const KNOWLEDGE_BASE = [
  // ── DAY 01 ENTRIES ────────────────────────────────────────────────────────
  {
    ...
    followup: 'Muốn hiểu thêm 4 cấp độ Agent xây dựng trên LLM? Xem [slide 23-24]'
  },
  {
    id: 'token-tokenomics',
    dayCode: 'Day01',
...
//  FORMAT RICH ANSWER
// ─────────────────────────────────────────────────────────────────────────────
const resolvePdfPage = (slide) => slide?.pdfPage || slide?.page;
...
const formatRichAnswer = (entry, currentDay) => {
  ...
  const normalizedCitations = entry.slidePages.map(p => {
    const dayLabel = entry.dayCode === 'Day01' ? 'Day 01' : entry.dayCode === 'Day02' ? 'Day 02' : entry.dayCode;
    return `${dayLabel} - slide ${p}`;
  });
  ...
  const slideDetails = entry.slidePages
    .map(p => {
      const slide = slideDay.slides?.find(s => resolvePdfPage(s) === p || s.page === p);
      return slide ? `- **[slide ${p}]** — *${slide.title}*` : `- **[slide ${p}]**`;
    })
    .filter(Boolean);
...
const fallbackRagSearch = (question, currentDay) => {
  ...
  const primaryPage = resolvePdfPage(primary.slide);
  const dayLabel = (code) => (code === 'Day01' ? 'Day 01' : code === 'Day02' ? 'Day 02' : code);

  let answer = `### ${primary.slide.title} [slide ${primaryPage}] — ${primary.dayItem.code}\n\n`;
  ...
  if (supplementary.length > 0) {
    ...
    for (const { slide, dayItem } of supplementary) {
      const sp = resolvePdfPage(slide);
      answer += `- **[slide ${sp}]** *${slide.title}* (${dayItem.code}): ${slide.content.split('\n')[0]}\n`;
    }
  }

  const citations = top.filter(s => s.score >= 3).map(s => `${dayLabel(s.dayItem.code)} - slide ${resolvePdfPage(s.slide)}`);
...
export const llmService = {
  /**
   * Learning Bridge — random sample từ ngân hàng 200 mẫu cố định.
   * Path lowConfidence / failure / boundary vẫn dùng kịch bản demo riêng.
   */
  generateLearningBridge: async ({ fromDay, toDay, pathMode = 'happy', forceMock = false }) => {
    ...
    if (pathMode !== 'happy') {
      ...
      logger.logLLMCall({
        prompt: `Path demo bank: ${fromDay.code}->${toDay.code} [${pathMode}] forceMock=${!!forceMock}`,
        model: 'experience-path',
        isRealAPI: false,
        pathMode,
        agent: 'BridgeAgent'
      }, mockResult, Date.now() - startTime);
      return { ...mockResult, isRealAPI: false, timestamp: Date.now() };
    }

    const sampled = sampleBridgeBundle({
      recap: 5,
      bridgeLinks: 3,
      checklist: 3,
      quiz: 3
    ...
    logger.logLLMCall({
      prompt: `BankSampler Bridge: ${fromDay.code}->${toDay.code} | pool=${BANK_STATS.total} samples | pick recap/bridge/checklist/quiz`,
      model: 'content-bank-v1',
      isRealAPI: false,
      pathMode: 'happy',
      agent: 'BridgeAgent'
    }, {
      picked: {
        recap: finalResult.recap.length,
        bridgeLinks: finalResult.bridgeLinks.length,
        checklist: finalResult.checklist.length,
        quiz: finalResult.quiz.length
      },
  ...
  /**
   * Quiz — random 3 câu từ QUIZ_BANK (60 mẫu).
   */
  generateBridgeQuiz: async ({ fromDay, toDay, count = 3 }) => {
    ...
    const result = {
      ...
      bankSize: BANK_STATS.quiz
    };

    logger.logLLMCall({
      prompt: `BankSampler Quiz: ${fromDay.code}->${toDay.code} | pool=${BANK_STATS.quiz} | pick=${quiz.length}`,
      model: 'quiz-bank-v1',
      isRealAPI: false,
      pathMode: 'quiz',
      agent: 'QuizAgent'
  ...
  answerTutorQuestion: async ({ day, question }) => {
    ...
    if (kbMatch) {
      ...
      logger.logLLMCall({ prompt: `KB: ${activeQuestion} -> ${kbMatch.entry.id}`, model: 'knowledge-base', isRealAPI: false, agent: 'TutorAgent', pathMode: 'tutor' }, richAnswer, Date.now() - startTime);
    ...
    if (ragResult) {
      ...
      logger.logLLMCall({ prompt: `RAG: ${activeQuestion}`, model: 'fallback-rag', isRealAPI: false, agent: 'TutorAgent', pathMode: 'tutor' }, ragResult, Date.now() - startTime);
